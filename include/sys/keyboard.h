#ifndef	_KOS_KEYBOARD_H_
#define	_KOS_KEYBOARD_H_

#define KB_IN_BUFFER_EMPTY  0x02

#define	LEN_KB_IN_BUFFER	32	/* size of keyboard input buffer */
...
struct KeyboardInBuffer {
    char*   p_head;                 /* Points to the next available position in the buffer */
    char*   p_tail;                 /* Points to the byte the keyboard task should process */
    int     count;                  /* Number of bytes in the buffer */
    char    buf[LEN_KB_IN_BUFFER];  /* Buffer */
};

/* Define the KeyboardState struct */
struct KeyboardState {
	int code_with_E0;
    int count;
    int shift_l, shift_r, alt_l, alt_r, ctrl_l, ctrl_r;
    int caps_lock, num_lock, scroll_lock;
    int column;
};

#endif /* _KOS_KEYBOARD_H_ */
